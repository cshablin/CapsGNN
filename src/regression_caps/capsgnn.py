...
class CapsGNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for layer in self.base_layers:
            ...
            #print('size of features and edges',len(features),len(edges))
            #print(features)
...
class CapsGNNTrainer(object):
    ...
    def test_mse(self):
            ...
            for path in tqdm(self.test_graph_paths):
                ...
                #prediction = prediction.reshape([prediction.size()[0]])
                target = data["target"]
                mse_loss = mse_loss(prediction,target,self.args.lambd)
                    #torch.nn.MSELoss().forward(prediction,target)
